...
static int shm_mmap(vfs_fd_t *fd, off_t offset, vmm_seg_t *seg) {
	...
	if (offset + VMM_SIZE(seg) > PAGE_ALIGN_UP(file->size))return -EINVAL;

	off_t off = 0;
	uintptr_t vaddr = seg->start;
	foreach(node, &file->blocks) {
		shm_block_t *block = (shm_block_t *)node;
		if (off >= offset && (size_t)off <= offset + VMM_SIZE(seg)) {
			if (vaddr >= seg->end)break;
