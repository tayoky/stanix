...
static gdt_segment create_gdt_segment(uint64_t base, uint64_t limit, uint8_t access, uint8_t flags) {
	gdt_segment result;
...
void init_gdt(void) {
	...
	// user code and data
	gdt[3] = create_gdt_segment(0, 0, GDT_SEGMENT_ACCESS_USER | GDT_SEGMENT_ACCESS_EXECUTABLE, 0x02);
	gdt[4] = create_gdt_segment(0, 0, GDT_SEGMENT_ACCESS_USER, 0x00);
