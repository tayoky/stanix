...
gdt_segment create_gdt_segement(uint32_t base,uint32_t limit,uint8_t access,uint8_t flags){
	gdt_segment result;
...
void init_gdt(void){
	...
	//tss
	kernel->arch.gdt[5] = create_gdt_segement((uint32_t)&kernel->arch.tss & 0xFFFFFFFF,sizeof(TSS) - 1,0x89,0);

	//create the GDTR and load it so the GDT is actually used
	kernel->arch.gdtr.size = sizeof(kernel->arch.gdt) - 1;
	kernel->arch.gdtr.offset = (uint32_t)&kernel->arch.gdt[0];
