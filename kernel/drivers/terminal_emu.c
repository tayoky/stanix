...
void init_terminal_emualtor(void){
	...
	vfs_fd_t *frambuffer_dev = vfs_open(frambuffer_path,VFS_WRITEONLY);
	if(!frambuffer_dev){
		kfail();
	...
	//now open the file
	vfs_fd_t *font_file = vfs_open(font_path,VFS_READONLY);
	if(!font_file){
		kfail();
	...
	vfs_fd_t *tty = vfs_open("/dev/tty0",VFS_WRITEONLY);
