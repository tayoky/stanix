...
tinx_build_source_cache () {
	...
	echo "# automatically generated by tinx
...
CACHED_TAG=\"$TAG\"
CACHED_DIR=\"$DIR\"
CACHED_DIR_HASH=\"$DIR_HASH\"
CACHED_PATCHES_HASH=\"$PATCHES_HASH\"" > "$CACHE/$1.sh"
}
...
tinx_is_source_cache_old () {
	...
	test "$CACHED_TAR" != "$TAR" ||
	test "$CACHED_GIT" != "$GIT" ||
	test "$CACHED_COMMIT" != "$COMMIT" ||
	test "$CACHED_TAG" != "$TAG" ||
	test "$CACHED_DIR" != "$DIR" ||
	test "$CACHED_DIR_HASH" != "$DIR_HASH" ||
	test "$CACHED_PATCHES_HASH" != "$PATCHES_HASH" ; then
		return 0
	else
		return 1
	fi
...
CACHED_SOURCE_HASH=\"$SOURCE_HASH\"" > "$TINX_CACHE/$1.sh"
}
...
tinx_is_package_cache_old () {
	...
	test "$CACHED_DEPENDENCIES" != "$DEPENDENCIES" ||
	test "$CACHED_SOURCE_HASH" != "$SOURCE_HASH" ; then
		return 0
	else
		return 1
	fi
...
tinx_select_package () {
	...
	if test -n "$SOURCE" ; then
		if ! . "sources/$SOURCE/$SOURCE.sh" ; then
